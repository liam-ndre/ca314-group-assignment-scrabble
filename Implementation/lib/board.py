...
#  expression (or if a given regular expression matches a particular string, 
...
class Board:
	...
	def valid_range(self, range_of_words, word, direction):
		for i, square in enumerate(range_of_words):
			if i == 0:
				if direction == 'd':
					if self.taken(square, 'r', self.up_or_left):
						return False
				else:
					if self.taken(square, 'd', self.up_or_left):
						return False

			if i == len(range_of_words) - 1:
				if direction == 'd':
					if self.taken(square, 'r', self.down_or_right):
						return False
				else:
					if self.taken(square, 'd', self.down_or_right):
						return False
	...
	def place(self, letters, range_of_words):
		for letter, square in zip(letters, range_of_words):
	...
	def taken(self, square, direction, func):
		# Find the ascii value of the letter on the square. There might be a bonus identifier on the spot.
		# Just in case we grab the first character. If the square is out of range of the board, '.' is given
	...
	def square_taken(self, square, direction):
		down_or_right_taken = self.taken(square, direction, self.down_or_right)
		up_or_left_taken = self.taken(square, direction, self.up_or_left)

		return down_or_right_taken or up_or_left_taken

	def __square_up(self, square): 							# Number part of a spot increases as it goes up
		if len(square) == 2:
			return square[0] + str(int(square[1]) + 1)
		else:
			return square[0] + str(int(square[1:]) + 1)

	def __square_down(self, square):						# Number part of a spot decreases as it goes up
		if len(square) == 2:
			return square[0] + str(int(square[1]) - 1)
		else:
			return square[0] + str(int(square[1:]) - 1)
